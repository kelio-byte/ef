...
- 新增参数：`n_children`，默认值先根据兼容性决定为 1 或 4。
...
- [ ] 实现稳定的 64 位 seed 混合函数。
- [ ] child seed 由 `(parent_seed, step, child_index)` 决定。
- [ ] child seed 不依赖 batch 排列。
- [ ] 增加碰撞和可复现性测试。

### 验收标准

- [ ] `M=1` 能退化到单后继行为。
- [ ] 扩展前模型 batch 规模是 K，而不是 K×M。
- [ ] 扩展后候选数量是 K×M。
- [ ] Top-K 剪枝第一次真正处理多于 K 个候选。
- [ ] 运行固定基准并记录 K=5、M=1/2/4 的结果。
