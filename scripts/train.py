...
from edit_flows.core.alignment import opt_align_xs_to_zs, identity_align_xs_to_zs
...
def initialize_distributed(device_arg: str) -> DistributedContext:
    ...
    if world_size == 1:
        ...
        return DistributedContext(
            rank=0, world_size=1, local_rank=0, device=requested_device
        )
    if requested_device.type == "cuda":
        if not torch.cuda.is_available():
            raise RuntimeError(
        ...
        if local_rank < 0 or local_rank >= torch.cuda.device_count():
            raise RuntimeError(
                f"LOCAL_RANK={local_rank} has no visible CUDA device; visible devices={torch.cuda.device_count()}"
            )
...
def _build_split_loader(
    ...
    if raw_exists[0] != raw_exists[1]:
        raise FileNotFoundError(f"Incomplete raw {split} files: {raw_src}, {raw_tgt}")
    if all(aligned_exists):
        ...
        align_fn = {"opt": opt_align_xs_to_zs}[align_name]
        source_kind = "raw"
    else:
        raise FileNotFoundError(
            f"No usable {split} pair found in {split_dir}; expected {aligned_src}/{aligned_tgt} or {raw_src}/{raw_tgt}"
        )
    if distributed is not None and distributed.is_distributed:
        if shuffle and seed is None:
            raise ValueError(
                "DDP training requires retro.seed so every rank uses the same per-epoch permutation"
            )
        ...
        sampler = (
            EpochRandomSampler(dataset, seed) if shuffle and seed is not None else None
        )
    ...
    *,
    ...
    if torch.cuda.is_available() and cuda_device is None:
        if "cuda_device_rng" in state:
            raise ValueError(
                "checkpoint has per-device CUDA RNG state but no CUDA device"
            )
        if "cuda" in state:
        ...
        if "cuda_device_rng" in state:
            ...
            if isinstance(cuda_states, (list, tuple)):
                ...
                if index >= len(cuda_states):
                    raise ValueError(
                        f"checkpoint CUDA RNG state has fewer devices than requested device index {index}"
                    )
                torch.cuda.set_rng_state(
                    _as_cpu_rng_state(cuda_states[index]), cuda_device
                )
            else:
                torch.cuda.set_rng_state(_as_cpu_rng_state(cuda_states), cuda_device)
        ...
        for key, name in (
            ...
            if key in metrics:
                writer.add_scalar(
                    ...
                    metrics[key] / metrics["u_tot"],
                    step,
                )
    for key, name in (
        ("lambda_total", "total"),
        ("lambda_ins", "insert"),
        ("lambda_sub", "substitute"),
        ("lambda_del", "delete"),
    ):
        if key in metrics:
...
def gradient_diagnostics(model) -> dict:
    ...
    for parameter in model.parameters():
        parameter_finite = torch.isfinite(parameter)
        if not parameter_finite.all():
            nonfinite_parameter_values += int((~parameter_finite).sum().item())
...
def prune_checkpoints(save_dir: str, keep: int) -> None:
    ...
    ckpts = sorted(
        ...
        key=lambda p: int(re.search("step(\\d+)", p).group(1)),
    )
...
def save_checkpoint(
    ...
    *,
    ...
    state = {
        ...
        "rng_state": (
            capture_rng_state(train_generator) if rng_state is None else rng_state
        ),
    }
...
def run_training(args, context: DistributedContext) -> None:
    ...
    for key, value in formal.items():
        if cfg.get(key) != value:
            raise ValueError(f"Formal training requires {key}={value!r}")
    device = context.device
    seed = cfg.get("seed")
    if seed is not None:
        seed = int(seed)
    ...
    validation_interval = int(
        tensorboard_cfg.get("validation_interval", cfg.get("validation_interval", 0))
    )
    ...
    need_validation = validation_interval > 0 and validation_start_step <= int(
        cfg["total_steps"]
    )
    ...
    if context.is_main_process:
        ...
        if getattr(args, "run_dir", None):
            ...
            proposed_save_dir = os.path.join("training_runs", dataset_name, timestamp)
    else:
    ...
    with log_context:
        ...
        rank_zero_print(
            context, f"Vocab: {real_vocab_size} real tokens, {model_vocab} model tokens"
        )
        (train_dataset, train_loader, align_fn, train_source_kind, train_sampler) = (
            _build_split_loader(
                data_dir=data_dir,
                split="train",
                token2id=token2id,
        ...
        if need_validation and any((os.path.exists(path) for path in val_paths)):
            (val_dataset, val_loader, val_align_fn, val_source_kind, val_sampler) = (
                _build_split_loader(
                    data_dir=data_dir,
                    split="val",
                    token2id=token2id,
        ...
        base_model = EditFlowsTransformer(
            ...
            use_product_memory=cfg.get("use_product_memory", False),
            product_memory_encoder_layers=cfg.get("product_memory_encoder_layers", 0),
        ...
        optimizer = torch.optim.Adam(
            model.parameters(), lr=0.0, betas=(0.9, 0.998), eps=1e-08
        )
        ...
        rank_zero_print(
            context,
            f"Train: {len(train_dataset):,} pairs, {len(train_loader):,} batches/epoch/rank",
        )
        if val_loader is not None:
            rank_zero_print(
                context,
                f"Validation: {len(val_dataset):,} pairs, source={val_source_kind}, batches/rank={len(val_loader):,}",
            )
        else:
        ...
        rank_zero_print(
            context,
            f"Topology: world_size={context.world_size}, backend={training_topology['backend']}, batch/rank={per_rank_batch_size}, effective global batch={effective_global_batch_size}",
        )
        ...
        rank_zero_print(
            context,
            f"Clamp kappa: {cfg.get('clamp_kappa', False)} (max={cfg.get('clamp_max', 50.0)})",
        )
        if cfg.get("use_product_memory", False):
            rank_zero_print(
                context,
                f"Product memory: enabled (encoder_layers={cfg.get('product_memory_encoder_layers')}, fusion_after_layers={cfg.get('product_memory_fusion_after_layers')})",
            )
        else:
        ...
        if resume_checkpoint is not None:
            ...
            lr_scheduler.load_state_dict(
                resume_checkpoint.get("lr_scheduler_state", {})
            )
            start_step = int(
                resume_checkpoint.get(
                    "completed_steps", resume_checkpoint.get("step", 0)
                )
            )
            ...
            if (
                ...
                raise ValueError(
                    f"checkpoint effective global batch size ({saved_global_batch}) differs from this run ({effective_global_batch_size}); start a new run instead"
                )
            if (
                ...
                rank_zero_print(
                    context,
                    "WARNING: resuming with a different world_size; effective batch matches, but this is not bitwise-identical continuation.",
                )
            ...
            if isinstance(rng_states, list) and len(rng_states) == context.world_size:
                ...
                context.world_size == 1 and isinstance(rng_states, list) and rng_states
            ):
                restore_rng_state(rng_states[0], train_generator, cuda_device=device)
                rank_zero_print(
                    context,
                    "WARNING: resumed rank-0 RNG from a multi-rank checkpoint; continuation is not bitwise identical.",
                )
            elif "rng_state" in resume_checkpoint:
                if context.is_distributed and context.rank != 0:
                    rank_zero_print(
                        context,
                        "WARNING: checkpoint has only single-process RNG state; rank 1+ keep deterministic rank-local seeds.",
                    )
                else:
                    restore_rng_state(
                    ...
                    rank_zero_print(
                        context, "Restored Python/NumPy/PyTorch/DataLoader RNG state"
                    )
            else:
                rank_zero_print(
                    context,
        ...
        if start_step > total_steps:
            raise ValueError(
                f"checkpoint completed_steps={start_step} exceeds configured total_steps={total_steps}"
            )
        writer = None
        if context.is_main_process and tensorboard_cfg.get("enabled", False):
            try:
                ...
                raise RuntimeError(
                    "TensorBoard is enabled but unavailable; install tensorboard with `pip install tensorboard`"
                ) from exc
            ...
            writer.add_text(
                "run/config", yaml.safe_dump(config, sort_keys=False), start_step
            )
            rank_zero_print(context, f"TensorBoard: {tensorboard_dir}")
        log_interval = int(
            tensorboard_cfg.get("log_interval", cfg.get("log_interval", 100))
        )
        ...
        if train_batches_per_epoch <= 0:
            raise RuntimeError(
                "Training loader has no complete batches; reduce batch_size or provide more training examples"
            )
        if (
            ...
            raise ValueError(
                "checkpoint was created with a different effective batch size; start a new run instead of silently changing the data order"
            )
        ...
        try:
            for step in range(start_step, total_steps):
                ...
                if log_due and context.is_main_process:
                    ...
                    rank_zero_print(
                        context,
                        f"step {completed_steps:>8}/{total_steps} | loss: {reported_metrics['loss']:.4f} | lr: {current_lr:.2e} | u_tot: {reported_metrics['u_tot']:6.2f} | ins: {reported_metrics['u_ins']:6.2f} | del: {reported_metrics['u_del']:6.2f} | sub: {reported_metrics['u_sub']:6.2f}",
                    )
                if val_loader is not None and validation_due(
                    completed_steps, validation_start_step, validation_interval
                ):
                    ...
                    if context.is_main_process:
                        ...
                        rank_zero_print(
                            context,
                            f"validation step {completed_steps:>8} | loss: {val_metrics['loss']:.4f} | u_tot: {val_metrics['u_tot']:6.2f} | ins: {val_metrics['u_ins']:6.2f} | del: {val_metrics['u_del']:6.2f} | sub: {val_metrics['u_sub']:6.2f}",
                        )
                ...
                if (
                    writer is not None
                    and completed_steps
                    % int(tensorboard_cfg.get("flush_interval", 500))
                    == 0
                ):
                    writer.flush()
            ...
            if monitoring_enabled and context.is_main_process:
                ...
                completed_this_run = total_steps - start_step
                summary = {
                    "schema_version": 1,
                    "status": "completed",
                    ...
                    "steps_per_second": (
                        completed_this_run / elapsed_seconds
            ...
            rank_zero_print(context, "=" * 55)
            rank_zero_print(
                context, f"Training complete. Final model saved to {final_path}"
            )
        finally:
            if writer is not None:
                writer.close()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="cpu",
        help="Single-process device. Under torchrun with --device cuda, each rank is automatically pinned to its LOCAL_RANK GPU.",
    )
    parser.add_argument(
        "--checkpoint", type=str, default=None, help="Resume from checkpoint (.pt path)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Max checkpoints to keep (default: training config)",
    )
