    ...
    try:
        for batch_idx in tqdm(range(n_batches), desc="Batches"):
            ...
            if use_greedy_beam:
                ...
                n_rep = args.n_runs  # same semantics as --n_samples
            else:
            ...
            if args.sampler == "euler":
                ...
                # Run beam_runs independent beams per product, each with
                # a different base_seed.  Output format matches --n_samples
                # so score.py can use the same --beam_size / --augmentation.
                B_prod = end - start
                all_results = []
                for r in range(args.n_runs):
                    idx_start = r * B_prod
                    idx_end = idx_start + B_prod
                    x_0_run = x_0[idx_start:idx_end]
                    run_seed = args.seed + r * 1000
                    res = sample_euler_beam(
                        model, x_0_run, kappa_scheduler,
                    ...
                    all_results.append(res.cpu())
                # Pad to uniform length (different beam runs = different lengths)
                max_len = max(r.shape[1] for r in all_results)
                padded = []
                for r in all_results:
                    if r.shape[1] < max_len:
                        p = torch.full((r.shape[0], max_len), PAD_TOKEN, dtype=torch.long)
                        p[:, :r.shape[1]] = r
                        padded.append(p)
                    else:
                        padded.append(r)
                results = torch.cat(padded, dim=0)
