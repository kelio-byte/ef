"""用途：准备训练批次，并执行单步训练或验证。
输入：模型、源目标序列、对齐结果和优化器等训练状态。
输出：损失与训练指标；训练时同时更新模型参数。
"""
...
from edit_flows.core.z_space import rm_gap_tokens, make_ut_mask_from_z, sample_cond_zt
...
def prepare_batch(
    ...
    z_t, _ = sample_cond_zt(
        z_0_padded, z_1_padded, t, model_vocab_size, scheduler, return_pick=True
    )
    ...
    if use_product_memory:
        ...
        # exactly the same representation built by inference.make_batch.
        # This also retains source tokens that disappear
...
def _forward_loss_and_metrics(
    ...
    log_rates, log_ins_probs, log_sub_probs = model(
        x_t, t_model, x_pad_mask, **model_kwargs
    )
    ...
    with torch.no_grad():
        ...
        log_ux_cat_eff = torch.cat(
            [log_u_tia_ins_eff, log_u_tia_sub_eff, log_u_tia_del_eff],
            dim=-1,
        )
